...
terraform plan -no-color -refresh=true -lock=true -destroy -state=$ComponentStateFile -out=$ComponentTemplatePlan \ 
    -var "resourceGroupName=$ComponentResourceGroup" \
    -var "resourceGroupLocation=$(az group show -n $ComponentResourceGroup --query location -o tsv)"
